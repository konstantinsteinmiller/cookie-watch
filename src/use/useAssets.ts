...
// Cookie Watch draws all gameplay art programmatically (via Canvas 2D)
...
const runBackgroundWarmup = (): void => {
  ...
  void (async () => {
    ...
    let art: typeof import('@/use/useCookieArt') | null = null
    try { art = await import('@/use/useCookieArt') } catch { /* non-critical */ }

    // 2. Parchment ribbon — before any optional sprite overrides.
    try { await art?.warmImages?.([RIBBON_SRC]) } catch { /* non-critical */ }

    // 3. Optional sprite overrides (cookie / mouse / cat) — only used if the
    //    matching files exist in /public/images; otherwise the renderer stays
    //    on its procedural vectors. No-op when absent.
    try { await art?.warmTileImages?.() } catch { /* non-critical */ }
  })()
}
...
export default () => {
  const preloadAssets = async (): Promise<void> => {
    ...
    const stageTask = (async () => {
      try {
        const art = await import('@/use/useCookieArt')
        await art.warmTileImages?.()
      } catch { /* non-critical: renderer falls back to procedural vectors */ }
    })()
