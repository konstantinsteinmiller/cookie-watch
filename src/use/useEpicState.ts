...
/**
 * Single-blob localStorage layer for Cookie Watch.
 *
 * EVERY persisted gameplay value the game touches lives inside ONE in-memory
 * record — `cookieState` — and only ONE localStorage key is ever written:
 * `cookie_state`. The cloud-sync subsystem (SaveManager / strategies) only
 ...
 * were reused wholesale from the Cookie Watch platform layer keep their original
...
export const STATE_KEY = 'cookie_watch_state'
 ...
 *  and writes `cookie_state` to localStorage synchronously. Called from
...
export const cookieState: Ref<Record<string, unknown>> = ref(buildInitial())
...
export const getState = <T = unknown>(key: string, fallback?: T): T => {
  const v = cookieState.value[key]
  return (v === undefined ? fallback : v) as T
}

export const hasState = (key: string): boolean => cookieState.value[key] !== undefined

export const setState = (key: string, value: unknown): void => {
  cookieState.value = { ...cookieState.value, [key]: value }
  schedulePersist()
}

export const removeState = (key: string): void => {
  if (cookieState.value[key] === undefined) return
  const next = { ...cookieState.value }
  delete next[key]
  cookieState.value = next
  schedulePersist()
}
...
export const reloadEpicState = (): void => {
  cookieState.value = buildInitial()
}
